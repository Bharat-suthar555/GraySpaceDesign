...
export function SiteHeader() {
  return (
    <motion.header
      ...
      className="fixed inset-x-0 top-0 z-50 border-b border-line bg-ink/72 backdrop-blur-md"
    >
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-10 md:flex" aria-label="Main navigation">
          {nav.map((item, index) => (
            <a
              key={item.href}
              href={item.href}
              className="group relative font-mono text-[11px] uppercase tracking-[0.18em] text-studio/74 transition-colors duration-300 hover:text-studio"
            >
              {item.label}
              <motion.span
        ...
        <a
          href="tel:8094325459"
          className="border border-line px-4 py-2 font-mono text-[11px] uppercase tracking-[0.16em] text-studio/86 transition-colors duration-300 hover:border-blueprint hover:text-studio"
        >
          Call
        </a>
      </div>
